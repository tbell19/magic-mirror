class Box{
  ...
  checkClick(){
    if(!mouseIsPressed){this.held = false}
  ...
  text(text,size,x,y){
    this.elements[this.elements.length] = new boxText(this,this.elements.length,text,size,x,y);
...
class boxText{
  constructor(box,id,text,size,x,y){
    this.box = box
    this.text = text
    this.size = size
    this.x = x
